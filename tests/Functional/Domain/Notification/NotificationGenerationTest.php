    ...
    public function testGenerateNotificationForRequestStepChange()
    {
        ...
        $client->request('POST', $url, [
            'request' => [
                ...
                'applicant'   => [
                    'firstName' => $request->getApplicant()->getFirstName() ?? 'firstname',
                    'lastName'  => $request->getApplicant()->getLastName() ?? 'lastname',
                    'civility'  => $request->getApplicant()->getCivility() ?? RequestCivilityDictionary::CIVILITY_MISS,
                    'mail'      => $request->getApplicant()->getMail() ?? 'test1@example.org',
                ],
                'date'            => date('d/m/Y'),
                'concernedPeople' => [
                    'firstName' => $request->getConcernedPeople()->getFirstName() ?? 'first',
                    'lastName'  => $request->getConcernedPeople()->getLastName() ?? 'last',
                    'civility'  => $request->getConcernedPeople()->getCivility() ?? RequestCivilityDictionary::CIVILITY_MISS,
                    'mail'      => $request->getConcernedPeople()->getMail() ?? 'test@example.org',
                ],
